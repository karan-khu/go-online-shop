...
func (r *inventoryRepositoryImpl) Filling(userId string, itemId int, qty int) ([]*InventoryEntity, error) {
	...
	if err := r.db.Connect().CreateInBatches(inventoryList, len(inventoryList)).Error; err != nil {
...
func (r *inventoryRepositoryImpl) Removing(userId string, itemId int, limit int) error {
	if err := r.db.Connect().
		Model(&InventoryEntity{}).
		Update("ActiveStatus", "UNAVAILABLE").
